...
import styles from "./GridStagger.module.css";
...
export default function GridStagger(props: Props) {
  let initialBackgroundColor = "#f3b5a6";
  ...
  let minWidth = 300;
  let numColoumns = Math.floor(props.windowWidth / minWidth);
  let width = minWidth + (props.windowWidth % minWidth) / numColoumns;
  let sets = [0];
  ...
  return (
    <div
      className={styles.stagger_container}
    ...
    >
      {props.imageList.map((image, index) => {
        ...
        if (top - lastTop > 200) {
          ...
          lastTop = top;
          setTimeout(() => props.setFooterColor(image.dominant_color), 0);
        }
